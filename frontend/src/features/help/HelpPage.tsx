...
const SECTIONS: Section[] = [
  ...
  {
    titel: 'Уведомления',
    text: 'После входа колокольчик в верхней панели показывает число непрочитанных уведомлений. Нажми на него, чтобы открыть центр уведомлений Glocke. На страницах входа, регистрации и публичной справки колокольчик не показывается.',
    screenshotSlug: 'notifications',
    screenshotAlt: 'Колокольчик с числом непрочитанных уведомлений в верхней панели',
  },
  {
    titel: 'Экспорт данных',
    text: 'В настройках аккаунта есть два разных экспорта. Прямой JSON содержит только данные аккаунта Schlüssel. «ZIP всех сервисов» собирается в фоне из Schlüssel, Kuvert, Tafel, Zettel, Glocke, Schrank и Herold; каждый сервис делает снимок независимо, поэтому время внутри файлов может различаться. Если часть сервисов недоступна, можно скачать частичный архив и повторить только ошибки. В manifest.json указаны состав, время, размеры, контрольные суммы и ошибки. Архив доступен только владельцу, не кэшируется, удаляется после срока хранения и ограничен квотами. В нём находятся личные данные: храни и передавай файл как конфиденциальный.',
